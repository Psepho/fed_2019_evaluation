...
predictions <- readxl::read_excel("Polls - October 16.xlsx") %>% 
...
Our [predictions](http://www.psephoanalytics.ca/2019/10/using-our-agent-based-model-to-scenario.html) for the 2019 Federal race in Toronto were generated by our agent-based model that used demographic characteristics and results from previous elections.
...
For this analysis, we restrict the comparison to just the major parties, as they were the only parties for which we estimated vote share. We also just compare the actual results from the predictions of our base scenario. In the future, our work will focus much more on scenario planning to explain political campaigns. 

We start by just plotting the difference between the actual votes and the predicted votes at the party and district level.
...
ggplot2::ggplot(data = results, ggplot2::aes(difference)) +
  ...
  ggplot2::ylab("") +
  ggplot2::annotate("text", x = 0.1, y = 10, label = "Overestimated") +
  ggplot2::annotate("text", x = -0.1, y = 10, label = "Underestimated")
```
...
```{r map, fig.cap="The distribution of the difference between the predicted and actual proportion of votes by Electoral District and party.", message = FALSE, warning = FALSE, echo = FALSE}
